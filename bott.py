...
def start_bot(message):
    if message.text.lower() == "/start":
        ...
        google = types.InlineKeyboardButton(
            "Гугле", url="https://www.youtube.com/watch?v=dQw4w9WgXcQ"
        )
        ...
        bot.send_message(
            message.chat.id,
            "Здесь можно легко и быстро посмотреть фильмы в том жанре, который вам нужен",
        )
        ...
        bot.send_message(
            message.chat.id,
            "Я не знаю твоего имени, не мог бы ты ввести его? \nВведите имя:",
        )
        ...
        bot.send_message(
            message.chat.id, "Введите название города, который хотите удалить:"
        )
...
def city(message):
    ...
    bot.send_message(
        message.chat.id, "Введите название вашего любимого(ой) фильма/игры:"
    )
...
def film_game(message):
    ...
    bot.send_message(
        message.chat.id,
        "Спасбо за эту информацию, надеюсь она будет использываться хоть когда-нибудь",
    )
...
def question_btn(message):
    ...
    bot.send_message(
        message.from_user.id,
        "Вопрос: \nCколько весит язык синего кита?",
        reply_markup=keyboard,
    )
